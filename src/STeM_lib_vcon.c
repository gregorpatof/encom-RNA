...
// ----------------- global variables -------------------
struct big_global {
	struct ptindex ptorder[1100]; // for ordering vertices around each face // RJN 081008 changed 100 to 1100 and 200 2200 
	struct vertex centerpt[1100]; // center points for each contact face    // RJN 081008
	struct vertex poly[2200]; // polyhedron vertices                    // RJN 081008
	struct plane cont[1100]; // atom and contact plane information     // RJN 081008
	struct edgevector vedge[2200];
	...
	char radfilename[200]; // custom name for radii.dat file including path ADDED RJN 28.04.2008
};

// --------------------- function prototypes ----------------------

double spherical_arc(struct vertex ptAo, struct vertex ptB, struct vertex ptC,
		float rado);
double cosPQR(double ptP[], double ptQ[], double ptR[]);
char test_point_vcon(double ptX[], struct plane cont[], int NC, float rado,
		int planeA, int planeB, int planeC);
char order_faces(int atomzero, struct vertex poly[], struct vertex centerpt[],
		float rado, int NC, int NV, struct plane cont[],
		struct ptindex ptorder[]);
void project_points(struct vertex poly[], struct vertex centerpt[], float rado,
		int NC, int NV, struct plane cont[]);
int voronoi_poly2(struct atom PDB[], int atomzero, struct plane cont[],
		float rado, int NC, struct contactlist contlist[],
		struct big_global *big_g);
int add_vertex(struct vertex poly[], int vn, double coor[], int planeA,
		int planeB, int planeC);
void add_vedge(struct edgevector vedge[], int edgenum, struct plane cont[],
		int plane0, int plane1, int testplane, struct vertex poly[],
		int startpt);
int solve_3x3(double eq0[], double eq1[], double eq2[], double pt[]);
int solve_2xS(struct plane eq0, struct plane eq1, float rado, double pt0[],
		double pt1[]);
int calc_region(struct atom PDB[], int PDBtot, int atomlist[], int numatoms,
		 struct big_global *big_g, int all_atom,
		struct pdb_atom *strc, gsl_matrix *m);
void calc_areas(struct vertex poly[], struct vertex centerpt[], float rado,
		int NC, int NV, struct plane cont[], struct ptindex ptorder[],
		int atomzero);
int index_protein_vcon(int PDBtot, struct big_global *big_g);
void assign_radii(struct atom *PDB, long int PDBtot);
void save_areas(struct plane cont[], struct contactlist contlist[], int NC,
		int atomzero, struct big_global *big_g);
int get_contlist4(struct atom PDB[], int atomzero,
		struct contactlist contlist[], int PDBtot, float rado, int dim,
		struct big_global *big_g);
...
void save_seeds(struct plane cont[], struct vertex poly[], int NV, int atomzero,
		struct big_global *big_g);
void get_firstvert(struct plane cont[], int *planeA, int *planeB, int *planeC,
		int NC, int atomzero, struct big_global *big_g);
struct atom *read_PDB_dom(struct pdb_atom *strc, struct atom *PDB,
		int number_atom);
// ========================================================================
int vcon_file_dom(struct pdb_atom *strc, gsl_matrix *m, int allatom) {
	...
	struct big_global big_g;
	big_g.numcarec = 0;
	big_g.showbonded = 'N'; // default
	big_g.planedef = 'X'; // default
	big_g.normalize = 'N'; // default

	// strcpy(FILENAME,argv[1]);
	
	PDB = read_PDB_dom(strc, PDB, allatom);
	...
	for (atomi = 0; atomi < allatom; ++atomi) {
		PDB[atomi].vol = 0.0;
...
int vcon_file(char FILENAME[60], struct pdb_atom *strc, gsl_matrix *m,
		int allatom) {
	int atomi; // atom number of atomzero
	long int PDBtot; // number of PDB atoms
	...
	// initialize contact atom index
	big_g.ca_index = (long int*) malloc(PDBtot * sizeof(long int));
	big_g.ca_recsize = 5 * PDBtot;
	big_g.ca_rec = (struct ca_struct*) malloc(
			big_g.ca_recsize * sizeof(struct ca_struct));
	big_g.seed = (long int*) malloc(3 * PDBtot * sizeof(long int));
	if ((!big_g.ca_rec) || (!big_g.ca_index) || (!big_g.seed)) {
		printf("memory allocation error\n");
		exit(1);
	}
	...
	// assign protein atoms to boxes in cubic grid
	big_g.dim = index_protein_vcon(PDBtot, &big_g);

	// calc volumes for all protein atoms
	// printf("PDB TOT:%d\n",PDBtot);
	calc_region(PDB, PDBtot, PDBlist, PDBtot,&big_g,allatom,strc,m);
...
int calc_region(struct atom PDB[], int PDBtot, int atomlist[], int numatoms,
	...
	for (atomi = 0; atomi < numatoms; ++atomi) {
		...
		for (cai = 0; cai < NC; ++cai) {
			...
			if (big_g->cont[cai].flag != 'X') {
				areatot += big_g->cont[cai].area;
			}
		}
		SAS = areao - areatot;
	...
	// print SAS and contact areas
	 FILE *out_file;
	char vince_temp[100] = "vcon_temp.dat";
	int vince_patom;
	int vince_satom;
	float vince_area;
	int vince_pnode;
	...
	//out_file = fopen(vince_temp,"w"); 	

	for (atomi = 0; atomi < PDBtot; ++atomi) {
		currindex = big_g->ca_index[atomi];

		rado = PDB[atomi].radius + Rw;
		contnum = 1;
		//printf(" %5d%5s %4d%5s        SAS           %7.2f\n", PDB[atomi].atomnum,       // COM. OUT RJN 29.04.2008
	//	 fprintf(out_file," %5d%5s %4d%5s %c       SAS           %7.2f\n", PDB[atomi].atomnum,       // ADDED RJN 29.04.2008
	//	 PDB[atomi].atomname, PDB[atomi].resnum, PDB[atomi].res,
	//	 PDB[atomi].chn,                                                            // ADDED RJN 29.04.2008
	//	 (big_g->normalize=='Y')?(PDB[atomi].SAS*100.0/(rado*rado*4.0*PI)):PDB[atomi].SAS);

		vince_patom = PDB[atomi].atomnum;
		vince_pnode = pair[vince_patom];

		while (currindex != -1) {
			// normalize if required
			if (big_g->normalize == 'Y') {
				big_g->ca_rec[currindex].area *= (100.0
						/ (rado * rado * 4.0 * PI));
			}
			if (big_g->showbonded == 'Y') {
				//printf("                      %5d%5s %4d%5s %7.2f  %4.2f\n",                    // COM. OUT RJN 29.04.2008
				vince_satom = PDB[big_g->ca_rec[currindex].atom].atomnum;
				vince_area = big_g->ca_rec[currindex].area;
				vince_snode = pair[vince_satom];
				//printf("%d :: %d %d :: %d %f\n",vince_patom,vince_satom,vince_pnode,vince_snode,vince_area);
				gsl_matrix_set(m, vince_snode, vince_pnode, vince_area);
	//			fprintf(out_file,"                      %5d%5s %4d%5s %c %7.2f  %4.2f\n",                   // ADDED RJN 29.04.2008
	//			 PDB[big_g->ca_rec[currindex].atom].atomnum, PDB[big_g->ca_rec[currindex].atom].atomname,
	//			 PDB[big_g->ca_rec[currindex].atom].resnum, PDB[big_g->ca_rec[currindex].atom].res,      
	//			 PDB[big_g->ca_rec[currindex].atom].chn,                                           // ADDED RJN 29.04.2008
	//			 big_g->ca_rec[currindex].area, big_g->ca_rec[currindex].dist);
				++contnum;
			} else {
				// use distance check d<2.0 A to identify bonded contacts. always print S-S bonds.
				coorA = PDB[atomi].coor;
				coorB = PDB[big_g->ca_rec[currindex].atom].coor;
				if ((((coorA[0] - coorB[0]) * (coorA[0] - coorB[0])
						+ (coorA[1] - coorB[1]) * (coorA[1] - coorB[1])
						+ (coorA[2] - coorB[2]) * (coorA[2] - coorB[2])) > 4.0)
						|| ((PDB[atomi].atomname[1] == 'S')
								&& (PDB[big_g->ca_rec[currindex].atom].atomname[1]
										== 'S'))) {
	...
	//				 big_g->ca_rec[currindex].area, big_g->ca_rec[currindex].dist);       
			...
			currindex = big_g->ca_rec[currindex].prev;
		}
	}
	//fclose(out_file);
	return (0);
...
int voronoi_poly2(struct atom PDB[], int atomzero, struct plane cont[],
	...
	for (cai = 0; cai < NC; ++cai) {
		...
		if (big_g->planedef == 'B') { // bisection - original Voronoi procedure
			planedist = atomdist / 2.0;
		} else if (big_g->planedef == 'R') { // radical plane (Gellatly and Finney) - default.
			planedist = (atomdist * atomdist + (rado - Rw) * (rado - Rw)
					- (ca_ptr->radius) * (ca_ptr->radius)) / (2 * atomdist);
		} else { // extended radical plane (McConkey et al). 
			planedist = (atomdist * atomdist + rado * rado
					- (Rw + ca_ptr->radius) * (Rw + ca_ptr->radius))
					/ (2 * atomdist);
		}
	...
	get_firstvert(cont, &planeA, &planeB, &planeC, NC, atomzero, big_g);
	solve_3x3(cont[planeA].Ai, cont[planeB].Ai, cont[planeC].Ai, temppt);

	// add first vertex to vertex list
	add_vertex(big_g->poly, 0, temppt, planeA, planeB, planeC);
	...
	add_vedge(big_g->vedge, 0, cont, planeA, planeB, planeC, big_g->poly, 0);
	add_vedge(big_g->vedge, 1, cont, planeB, planeC, planeA, big_g->poly, 0);
	add_vedge(big_g->vedge, 2, cont, planeC, planeA, planeB, big_g->poly, 0);
	...
	while (1) {
		...
		while ((big_g->vedge[startedge].endpt >= 0)
				&& ((edgenum - startedge) > 0)) {
			++startedge;
		}
		...
		stp = &big_g->poly[big_g->vedge[startedge].startpt];
		V = big_g->vedge[startedge].V;
		planeA = big_g->vedge[startedge].plane[0];
		planeB = big_g->vedge[startedge].plane[1];
		oldplaneC = big_g->vedge[startedge].startplane;
		planeC = -1;
		...
		for (cai = 0; cai < NC; ++cai) {
			...
			if ((cai != planeA) && (cai != planeB) && (cai != oldplaneC)) {
				vtdiv = (cont[cai].Ai[0] * V[0] + cont[cai].Ai[1] * V[1]
						+ cont[cai].Ai[2] * V[2]);
				if (vtdiv != 0.0) {
					vt = -(cont[cai].Ai[0] * stp->xi[0]
							+ cont[cai].Ai[1] * stp->xi[1]
							+ cont[cai].Ai[2] * stp->xi[2] + cont[cai].Ai[3])
							/ vtdiv;
					if ((vt < vtmin) && (vt > 0)) {
						vtmin = vt;
						planeC = cai;
		...
		big_g->poly[vn].xi[0] = stp->xi[0] + vtmin * V[0];
		big_g->poly[vn].xi[1] = stp->xi[1] + vtmin * V[1];
		big_g->poly[vn].xi[2] = stp->xi[2] + vtmin * V[2];

		// if point is outside sphere, check vs. external planes
		if ((big_g->poly[vn].xi[0] * big_g->poly[vn].xi[0]
				+ big_g->poly[vn].xi[1] * big_g->poly[vn].xi[1]
				+ big_g->poly[vn].xi[2] * big_g->poly[vn].xi[2])
				> rado * rado) {
			for (cai = NC; cai < NC + 4; ++cai) {
				...
				if ((cai != planeA) && (cai != planeB) && (cai != oldplaneC)) {
					...
					if (vtdiv != 0.0) {
						vt =
								-(cont[cai].Ai[0] * stp->xi[0]
										+ cont[cai].Ai[1] * stp->xi[1]
										+ cont[cai].Ai[2] * stp->xi[2]
										+ cont[cai].Ai[3]) / vtdiv;
						if ((vt < vtmin) && (vt > 0)) {
							vtmin = vt;
							planeC = cai;
		...
		add_vertex(big_g->poly, vn, big_g->poly[vn].xi, planeA, planeB, planeC);
		big_g->vedge[startedge].endpt = vn;
		big_g->vedge[startedge].endplane = planeC;
		...
		edgei = startedge + 1;
		while (edgei < edgenum) {
			if (((big_g->vedge[edgei].plane[0] == planeA)
					&& (big_g->vedge[edgei].plane[1] == planeC))
					|| ((big_g->vedge[edgei].plane[0] == planeC)
							&& (big_g->vedge[edgei].plane[1] == planeA))) {
				// already on list, add current vertex as endpt
				big_g->vedge[edgei].endpt = vn;
				big_g->vedge[edgei].endplane = planeB;
				edgeflag = 'N';
				break;
			}
			++edgei;
		...
		if (edgeflag == 'Y') { // add edge
			add_vedge(big_g->vedge, edgenum, cont, planeA, planeC, planeB,
					big_g->poly, vn);
			++edgenum;
		}
		...
		edgei = startedge + 1;
		while (edgei < edgenum) {
			if (((big_g->vedge[edgei].plane[0] == planeB)
					&& (big_g->vedge[edgei].plane[1] == planeC))
					|| ((big_g->vedge[edgei].plane[0] == planeC)
							&& (big_g->vedge[edgei].plane[1] == planeB))) {
				// already on list, add current vertex as endpt
				big_g->vedge[edgei].endpt = vn;
				big_g->vedge[edgei].endplane = planeA;
				edgeflag = 'N';
				break;
			}
			++edgei;
		...
		if (edgeflag == 'Y') { // add edge
			add_vedge(big_g->vedge, edgenum, cont, planeB, planeC, planeA,
					big_g->poly, vn);
			++edgenum;
			...
			if (edgenum >= 200) {
				...
				big_g->seed[atomzero * 3] = -1; // reset to no seed vertex
				origcoor[0] = PDB[atomzero].coor[0];
				origcoor[1] = PDB[atomzero].coor[1];
				origcoor[2] = PDB[atomzero].coor[2];
				// perturb atom coordinates
				PDB[atomzero].coor[0] += 0.005 * (float) (2 * rand() - RAND_MAX)
						/ (float) RAND_MAX;
				PDB[atomzero].coor[1] += 0.005 * (float) (2 * rand() - RAND_MAX)
						/ (float) RAND_MAX;
				PDB[atomzero].coor[2] += 0.005 * (float) (2 * rand() - RAND_MAX)
						/ (float) RAND_MAX;
				recalc = 'Y';
				goto RESTART;
			}
	...
	for (edgei = 0; edgei < edgenum; ++edgei) {
		if ((rado < big_g->poly[big_g->vedge[edgei].startpt].dist)
				|| (rado < big_g->poly[big_g->vedge[edgei].endpt].dist)) {
			// one or both vertices fall outside of sphere
			arcflag = 'Y';
			big_g->vedge[edgei].arc = '?';
		} else {
			big_g->vedge[edgei].arc = 'X';
		}
	}

	// calculate new arc points
	for (edgei = 0; edgei < edgenum; ++edgei) {
		if (big_g->vedge[edgei].arc != 'X') {
			if (solve_2xS(cont[big_g->vedge[edgei].plane[0]],
					cont[big_g->vedge[edgei].plane[1]], rado, arcpt0, arcpt1)
					== -1) {
				big_g->vedge[edgei].arc = 'X'; // mark edge as no associated arc point
				continue;
			}
			// test new arc points vs. adjacent planes, add if ok.
			testpA = big_g->vedge[edgei].startplane;
			testpB = big_g->vedge[edgei].endplane;
			testvalA = cont[testpA].Ai[0] * arcpt0[0]
					+ cont[testpA].Ai[1] * arcpt0[1]
					+ cont[testpA].Ai[2] * arcpt0[2] + cont[testpA].Ai[3];
			testvalB = cont[testpB].Ai[0] * arcpt0[0]
					+ cont[testpB].Ai[1] * arcpt0[1]
					+ cont[testpB].Ai[2] * arcpt0[2] + cont[testpB].Ai[3];
			if ((testvalA < 0.0) && (testvalB < 0.0)) { // point is good
				add_vertex(big_g->poly, vn, arcpt0,
						big_g->vedge[edgei].plane[0],
						big_g->vedge[edgei].plane[1], -1);
				big_g->poly[vn].dist = rado;
				++vn;
			}
			testvalA = cont[testpA].Ai[0] * arcpt1[0]
					+ cont[testpA].Ai[1] * arcpt1[1]
					+ cont[testpA].Ai[2] * arcpt1[2] + cont[testpA].Ai[3];
			testvalB = cont[testpB].Ai[0] * arcpt1[0]
					+ cont[testpB].Ai[1] * arcpt1[1]
					+ cont[testpB].Ai[2] * arcpt1[2] + cont[testpB].Ai[3];
			if ((testvalA < 0.0) && (testvalB < 0.0)) { // point is good
				add_vertex(big_g->poly, vn, arcpt1,
						big_g->vedge[edgei].plane[0],
						big_g->vedge[edgei].plane[1], -1);
				big_g->poly[vn].dist = rado;
				++vn;
	...
	vj = 0;
	for (vi = 0; vi < vn; ++vi) {
		if (big_g->poly[vi].dist <= rado) {
			big_g->poly[vj] = big_g->poly[vi];
			++vj;
		}
	}
	vn = vj;
	...
	for (cai = 0; cai < NC; ++cai) {
		big_g->centerpt[cai].xi[0] = -cont[cai].Ai[0] * cont[cai].Ai[3];
		big_g->centerpt[cai].xi[1] = -cont[cai].Ai[1] * cont[cai].Ai[3];
		big_g->centerpt[cai].xi[2] = -cont[cai].Ai[2] * cont[cai].Ai[3];
		big_g->centerpt[cai].dist = fabs(cont[cai].Ai[3]);
	...
	if (recalc == 'N') {
		save_seeds(cont, big_g->poly, vn, atomzero, big_g);
	} else {
...
void get_firstvert(struct plane cont[], int *planeA, int *planeB, int *planeC,
	...
	*planeB = -1;
	*planeC = -1;
	if (big_g->seed[seedi] != -1) {
		for (cai = 0; cai < NC; ++cai) {
			if (cont[cai].index == big_g->seed[seedi]) {
				*planeA = cai;
			} else if (cont[cai].index == big_g->seed[seedi + 1]) {
				*planeB = cai;
			} else if (cont[cai].index == big_g->seed[seedi + 2]) {
				*planeC = cai;
			}
		}
	}
	if ((*planeA != -1) && (*planeB != -1) && (*planeC != -1)) {
		...
		for (cai = 0; cai < NC + 4; ++cai) {
			if (cai != *planeA) {
				vectA[0] = cont[*planeA].Ai[1] * cont[cai].Ai[2]
						- cont[*planeA].Ai[2] * cont[cai].Ai[1];
				vectA[1] = cont[*planeA].Ai[2] * cont[cai].Ai[0]
						- cont[*planeA].Ai[0] * cont[cai].Ai[2];
				vectA[2] = cont[*planeA].Ai[0] * cont[cai].Ai[1]
						- cont[*planeA].Ai[1] * cont[cai].Ai[0];
				vectA[3] = 0.0;
				if (solve_3x3(cont[*planeA].Ai, cont[cai].Ai, vectA, temppt)
						!= -1) {
					ptdist = sqrt(
							temppt[0] * temppt[0] + temppt[1] * temppt[1]
									+ temppt[2] * temppt[2]);
					if (ptdist < mindist) {
						*planeB = cai;
						mindist = ptdist;
		...
		vectA[0] = cont[*planeA].Ai[1] * cont[*planeB].Ai[2]
				- cont[*planeA].Ai[2] * cont[*planeB].Ai[1];
		vectA[1] = cont[*planeA].Ai[2] * cont[*planeB].Ai[0]
				- cont[*planeA].Ai[0] * cont[*planeB].Ai[2];
		vectA[2] = cont[*planeA].Ai[0] * cont[*planeB].Ai[1]
				- cont[*planeA].Ai[1] * cont[*planeB].Ai[0];
		vectA[3] = 0.0;
		...
		for (cai = 0; cai < NC + 4; ++cai) {
			if ((cai != *planeA) && (cai != *planeB)) {
				vtdiv = (cont[cai].Ai[0] * vectA[0] + cont[cai].Ai[1] * vectA[1]
						+ cont[cai].Ai[2] * vectA[2]);
				if (vtdiv != 0.0) {
					vt = -(cont[cai].Ai[0] * ptA[0] + cont[cai].Ai[1] * ptA[1]
							+ cont[cai].Ai[2] * ptA[2] + cont[cai].Ai[3])
							/ vtdiv;
					if (fabs(vt) < vtmin) {
						vtmin = fabs(vt);
						*planeC = cai;
...
void save_seeds(struct plane cont[], struct vertex poly[], int NV, int atomzero,
	...
	for (vi = 0; vi < NV; ++vi) {
		if (poly[vi].plane[2] != -1) {
			...
			if (big_g->seed[seedi] == -1) {
				big_g->seed[seedi] = atomzero;
				big_g->seed[seedi + 1] = cont[poly[vi].plane[1]].index;
				big_g->seed[seedi + 2] = cont[poly[vi].plane[2]].index;
			}
			seedi = 3 * cont[poly[vi].plane[1]].index;
			if (big_g->seed[seedi] == -1) {
				big_g->seed[seedi] = atomzero;
				big_g->seed[seedi + 1] = cont[poly[vi].plane[0]].index;
				big_g->seed[seedi + 2] = cont[poly[vi].plane[2]].index;
			}
			...
			if (big_g->seed[seedi] == -1) {
				big_g->seed[seedi] = atomzero;
				big_g->seed[seedi + 1] = cont[poly[vi].plane[0]].index;
				big_g->seed[seedi + 2] = cont[poly[vi].plane[1]].index;
			}
		}
	}
	return;
...
void add_vedge(struct edgevector vedge[], int edgenum, struct plane cont[],
	...
	vedge[edgenum].V[0] = cont[plane0].Ai[1] * cont[plane1].Ai[2]
			- cont[plane0].Ai[2] * cont[plane1].Ai[1];
	vedge[edgenum].V[1] = cont[plane0].Ai[2] * cont[plane1].Ai[0]
			- cont[plane0].Ai[0] * cont[plane1].Ai[2];
	vedge[edgenum].V[2] = cont[plane0].Ai[0] * cont[plane1].Ai[1]
			- cont[plane0].Ai[1] * cont[plane1].Ai[0];
	...
	testval = cont[testplane].Ai[0] * testpt[0]
			+ cont[testplane].Ai[1] * testpt[1]
...
int add_vertex(struct vertex poly[], int vn, double coor[], int planeA,
	...
	poly[vn].dist = sqrt(
			coor[0] * coor[0] + coor[1] * coor[1] + coor[2] * coor[2]);
	return (0);
}
...
char order_faces(int atomzero, struct vertex poly[], struct vertex centerpt[],
	...
	for (planeX = 0; planeX < NC; ++planeX) {
		...
		for (vi = 0; vi < NV; ++vi) {
			...
			if ((poly[vi].plane[0] == planeX) || (poly[vi].plane[1] == planeX)
					|| (poly[vi].plane[2] == planeX)) {
		...
		if (surfcount > 2) {
			...
			for (vi = 0; vi < surfcount; ++vi) {
				if (poly[ptorder[planeX].pt[vi]].plane[2] == -1) { // on arc, calc pt. opposite
					...
					temppt[0] = 2.0 * centerpt[planeX].xi[0]
							- poly[ptorder[planeX].pt[vi]].xi[0];
					temppt[1] = 2.0 * centerpt[planeX].xi[1]
							- poly[ptorder[planeX].pt[vi]].xi[1];
					temppt[2] = 2.0 * centerpt[planeX].xi[2]
							- poly[ptorder[planeX].pt[vi]].xi[2];

					//keep point if it's valid
					if (test_point_vcon(temppt, cont, NC, rado, planeX, -1, -1)
							== 'Y') {
		...
		if (surfcount > 3) {
			...
			for (vi = 1; vi < surfcount; ++vi) {
				if ((poly[ptorder[planeX].pt[vi]].plane[0] == planeY)
						|| (poly[ptorder[planeX].pt[vi]].plane[1] == planeY)
			...
			for (vi = 2; vi < surfcount; ++vi) {
				cos10X[vi] = cosPQR(poly[ptorder[planeX].pt[1]].xi,
						poly[ptorder[planeX].pt[0]].xi,
...
void calc_areas(struct vertex poly[], struct vertex centerpt[], float rado,
	...
	for (planeX = 0; planeX < NC; ++planeX) {
		...
		if (NP == 0) {
			if (test_point_vcon(centerpt[planeX].xi, cont, NC, rado, planeX, -1,
					-1) == 'Y') {
				cont[planeX].area = 2.0 * PI * rado
						* (rado - centerpt[planeX].dist);
			}
			...
			if (test_point_vcon(centerpt[planeX].xi, cont, NC, rado, planeX, -1,
				...
				cont[planeX].area = 2.0 * PI * rado
						* (rado - centerpt[planeX].dist)
						- spherical_arc(centerpt[planeX],
								poly[ptorder[planeX].pt[0]],
								poly[ptorder[planeX].pt[1]], rado);
			} else { // area is arc.
				cont[planeX].area = spherical_arc(centerpt[planeX],
						poly[ptorder[planeX].pt[0]],
						poly[ptorder[planeX].pt[1]], rado);
			}
		} else {
			...
			for (vi = 0; vi < NP; ++vi) {
				...
				cosNN1[vi] = ((poly[va].xi[0] * poly[vb].xi[0]
						+ poly[va].xi[1] * poly[vb].xi[1]
						+ poly[va].xi[2] * poly[vb].xi[2])
						/ (poly[va].dist * poly[vb].dist));

				// calculate cosines between vertex zero and vertex 'vi'
				if (vi != 0) {
					cosNzero[vi] = ((poly[v0].xi[0] * poly[va].xi[0]
							+ poly[v0].xi[1] * poly[va].xi[1]
							+ poly[v0].xi[2] * poly[va].xi[2])
							/ (poly[v0].dist * poly[va].dist));
				}
			}
			...
			for (vi = 0; vi < NP; ++vi) {
				...
				if ((poly[va].plane[2] == -1) && (poly[vb].plane[2] == -1)) {
					...
					if ((poly[va].plane[0] + poly[va].plane[1])
							!= (poly[vb].plane[0] + poly[vb].plane[1])) {
						area += spherical_arc(centerpt[planeX], poly[va],
								poly[vb], rado);
					}
				}
			}
	...
	if (engflag == 'Y') {
		for (planeX = 0; planeX < NC; ++planeX) {
			...
			for (vi = 0; vi < NP; ++vi) {
				...
				if ((pa1 == poly[nextpt].plane[0])
						|| (pa1 == poly[nextpt].plane[1])
						|| (pa1 == poly[nextpt].plane[2])) {
					commplane = pa1;
				} else if ((pa2 == poly[nextpt].plane[0])
						|| (pa2 == poly[nextpt].plane[1])
				...
				if ((commplane != -1) && (cont[commplane].flag != 'E')) {
					...
					cont[commplane].area += spherical_arc(centerpt[planeX],
							poly[currpt], poly[nextpt], rado);
					if (NP == 2)
						break; // otherwise would repeat adding area
				}
			}
		}
		...
		if (epi == 1) {
			cont[engplane[0]].area = 2.0 * PI * rado
					* (rado + cont[engplane[0]].dist);
		} else if (epi == 2) {
			if (solve_2xS(cont[engplane[0]], cont[engplane[1]], rado, ptB.xi,
					ptC.xi) == -1) {
...
void save_areas(struct plane cont[], struct contactlist contlist[], int NC,
	...
	if (big_g->numcarec > (big_g->ca_recsize - 100)) {
		big_g->ca_recsize += 10000;
		big_g->ca_rec = (struct ca_struct*) realloc(big_g->ca_rec,
				big_g->ca_recsize * sizeof(struct ca_struct));
		if (!big_g->ca_rec) {
			printf("memory allocation error\n");
			exit(1);
	...
	cai = 0;
	currindex = big_g->ca_index[atomzero];
	previndex = -1;
	while ((currindex != -1) && (cai < NC)) {
		if ((cont[cai].area > 0.0) && (cont[cai].flag != 'X')) {
			big_g->ca_rec[currindex].atom = cont[cai].index;
			big_g->ca_rec[currindex].area = cont[cai].area;
			big_g->ca_rec[currindex].dist = contlist[cai].dist;
			nextindex = big_g->ca_rec[currindex].prev; // next index is prev record from old list
			big_g->ca_rec[currindex].prev = previndex;
			previndex = currindex;
			currindex = nextindex;
		}
		++cai;
	}
	big_g->ca_index[atomzero] = previndex;

	// then add new records
	while (cai < NC) {
		if ((cont[cai].area > 0.0) && (cont[cai].flag != 'X')) {
			big_g->ca_rec[big_g->numcarec].atom = cont[cai].index;
			big_g->ca_rec[big_g->numcarec].area = cont[cai].area;
			big_g->ca_rec[big_g->numcarec].dist = contlist[cai].dist;
			big_g->ca_rec[big_g->numcarec].prev = big_g->ca_index[atomzero];
			big_g->ca_index[atomzero] = big_g->numcarec;
			++big_g->numcarec;
		}
		++cai;
	}
	...
	for (cai = 0; cai < NC; ++cai) {
		if (PDB[cont[cai].index].done != 'Y') {
			if ((cont[cai].area > 0.0) && (cont[cai].flag != 'X')) {
				big_g->ca_rec[big_g->numcarec].atom = atomzero;
				big_g->ca_rec[big_g->numcarec].prev =
						big_g->ca_index[cont[cai].index];
				big_g->ca_index[cont[cai].index] = big_g->numcarec;
				++big_g->numcarec;
			}
		}
	}
...
void project_points(struct vertex poly[], struct vertex centerpt[], float rado,
	...
	if (epi == 1) {
		...
		solve_3x3(cont[engplane[0]].Ai, cont[engplane[1]].Ai,
				cont[engplane[2]].Ai, pt0);
		projpt[0] = pt0[0];
		projpt[1] = pt0[1];
		projpt[2] = pt0[2];
	}
...
char test_point_vcon(double ptX[], struct plane cont[], int NC, float rado,
	...
	for (cp = 0; cp < NC; ++cp) {
		...
		if ((cp != planeA) && (cp != planeB) && (cp != planeC)
				&& (cont[cp].flag != 'X')) {
			if ((cont[cp].Ai[0] * ptX[0] + cont[cp].Ai[1] * ptX[1]
					+ cont[cp].Ai[2] * ptX[2] + cont[cp].Ai[3]) > 0.0) {
...
int solve_2xS(struct plane eq0, struct plane eq1, float rado, double pt0[],
	...
	t = (rado * rado - avgpt[0] * avgpt[0] - avgpt[1] * avgpt[1]
			- avgpt[2] * avgpt[2]) / det;
	if (t < 0) {
		return (-1);
	} else {
...
double cosPQR(double ptP[], double ptQ[], double ptR[]) {
	...
	cosine = (QP[0] * QR[0] + QP[1] * QR[1] + QP[2] * QR[2])
			/ sqrt(
					(QP[0] * QP[0] + QP[1] * QP[1] + QP[2] * QP[2])
							* (QR[0] * QR[0] + QR[1] * QR[1] + QR[2] * QR[2]));
...
double spherical_arc(struct vertex ptAo, struct vertex ptB, struct vertex ptC,
	...
	cosAOB = (ptAo.xi[0] * ptB.xi[0] + ptAo.xi[1] * ptB.xi[1]
			+ ptAo.xi[2] * ptB.xi[2]) / (ptAo.dist * ptB.dist);
	cosBOC = (ptB.xi[0] * ptC.xi[0] + ptB.xi[1] * ptC.xi[1]
			+ ptB.xi[2] * ptC.xi[2]) / (ptB.dist * ptC.dist);
	...
	area = rado
			* (angBAC * (rado - ptAo.dist) - 4.0 * rado * atan(sqrt(tanprod)));
	return (area);
}
...
struct atom *read_PDB_dom(struct pdb_atom *strc, struct atom *PDB,
		int number_atom) {
	int i;
	PDB = (struct atom*) malloc(number_atom * sizeof(struct atom));
	for (i = 0; i < number_atom; i++) {

		PDB[i].atomnum = strc[i].atom_number;
		strncpy(PDB[i].atomname, strc[i].atom_prot_type, 4); // atom name
		PDB[i].atomname[4] = '\0';
		strncpy(PDB[i].res, strc[i].res_type, 3); // residue name
		PDB[i].res[3] = '\0';

		PDB[i].chn = strc[i].chain[0]; // chain name ADDED RJN 29.04.2008

		PDB[i].resnum = strc[i].res_number; // residue number
		PDB[i].coor[0] = strc[i].x_cord; // x coordinate
		PDB[i].coor[1] = strc[i].y_cord; // y coordinate
		PDB[i].coor[2] = strc[i].z_cord; // z coordinate

		/*
		printf(
				"atomnum:%i atomname:%s res:%s chn:%c resnum:%i coor[0]:%f coor[1]:%f coor[2]:%f\n",
				PDB[i].atomnum, PDB[i].atomname, PDB[i].res, PDB[i].chn,
				PDB[i].resnum, PDB[i].coor[0], PDB[i].coor[1], PDB[i].coor[2]);
		*/
	}
	assign_radii(PDB, number_atom);
...
struct atom *read_PDB(char *FILE_ptr, struct atom *PDB, long int *numPDBatoms) {
	...
	PDB = (struct atom*) malloc(PDBarraysize * sizeof(struct atom));
	if (!PDB) {
	...
	while (1) {
		...
		if (PDBtot >= PDBarraysize) {
			PDBarraysize += 1000;
			PDB = (struct atom*) realloc(PDB,
					PDBarraysize * sizeof(struct atom));
			if (!PDB) {
				printf("memory allocation for PDB atoms failed\n");
				exit(1);
			}
		...
		if ((!strncmp(&oneline[0], "ATOM  ", 6))
				|| (!strncmp(&oneline[0], "HETATM", 6))) {
			//**********include this line if only ATOMs -->
			//if (!strncmp(&oneline[0], "ATOM  ", 6)) {

			// ------- skip alternate location records ----------
			if ((oneline[16] != ' ') && (oneline[16] != 'A'))
				continue;
			// ------- skip HOH records ----------
			if (!strncmp(&oneline[17], "HOH", 3))
				continue;
			...
			PDB[PDBtot].chn = oneline[21]; // chain name ADDED RJN 29.04.2008
...
void assign_radii(struct atom *PDB, long int PDBtot) {
	...
	//char   filename[20] = "radii.dat"; //** COMMENTED OUT BY RJN 28.04.2008
	...
	// ------- open file for input ---------
	// radfile = fopen(filename, "r"); //** COMMENTED OUT BY RJN 28.04.2008
	//printf("Radii Power\n");
	radius[0] = 1.61;
	radius[1] = 1.76;
	radius[2] = 1.88;
	...
	for (atomi = 0; atomi < PDBtot; ++atomi) {
		...
		if (!strncmp(PDB[atomi].res, "  C", 3)
				|| !strncmp(PDB[atomi].res, "  G", 3)
				|| !strncmp(PDB[atomi].res, "  A", 3)
				|| !strncmp(PDB[atomi].res, "  T", 3)
			...
			if (PDB[atomi].atomname[1] == 'P') {
				...
				// NITROGEN ATOM
				if ((PDB[atomi].atomname[2] == '2')
						|| (PDB[atomi].atomname[2] == '4')
						|| (PDB[atomi].atomname[2] == '6')) {
			...
			if (PDB[atomi].atomname[1] == 'O') {
				//   OXYGEN
				if ((PDB[atomi].atomname[2] == 'G')
						|| (PDB[atomi].atomname[2] == 'H')) {
				...
				if (!strncmp(PDB[atomi].res, "PRO", 3)) {
					...
					PDB[atomi].radius = radius[4]; // N3H1
				} else if ((PDB[atomi].atomname[2] == 'D')
						&& (!strncmp(PDB[atomi].res, "HIS", 3))) {
					PDB[atomi].radius = radius[4]; // N3H1
				} else if ((PDB[atomi].atomname[2] == 'D')
						&& (!strncmp(PDB[atomi].res, "ASN", 3))) {
					PDB[atomi].radius = radius[5]; // N3H2
				} else if ((PDB[atomi].atomname[2] == 'E')
						&& (!strncmp(PDB[atomi].res, "GLN", 3))) {
					PDB[atomi].radius = radius[5]; // N3H2
				} else if ((PDB[atomi].atomname[2] == 'E')
						&& (strncmp(PDB[atomi].res, "GLN", 3))) {
					PDB[atomi].radius = radius[4]; // N3H1
				...
				} else if ((!strncmp(PDB[atomi].res, "ASP", 3))
						&& (PDB[atomi].atomname[2] == 'G')) {
					PDB[atomi].radius = radius[0]; // C3H0
				} else if ((!strncmp(PDB[atomi].res, "GLU", 3))
						&& (PDB[atomi].atomname[2] == 'D')) {
					PDB[atomi].radius = radius[0]; // C3H0
				} else if ((!strncmp(PDB[atomi].res, "ASN", 3))
						&& (PDB[atomi].atomname[2] == 'G')) {
					PDB[atomi].radius = radius[0]; // C3H0
				} else if ((!strncmp(PDB[atomi].res, "GLN", 3))
						&& (PDB[atomi].atomname[2] == 'D')) {
					PDB[atomi].radius = radius[0]; // C3H0
				} else if ((!strncmp(PDB[atomi].res, "ARG", 3))
						&& (PDB[atomi].atomname[2] == 'Z')) {
					PDB[atomi].radius = radius[0]; // C3H0
				} else if ((!strncmp(PDB[atomi].res, "PHE", 3))
						|| (!strncmp(PDB[atomi].res, "HIS", 3))
						|| (!strncmp(PDB[atomi].res, "TYR", 3))
						|| (!strncmp(PDB[atomi].res, "TRP", 3))) {
					if ((PDB[atomi].atomname[2] == 'A')
							|| (PDB[atomi].atomname[2] == 'B')) {
						PDB[atomi].radius = radius[2]; // C4
					} else {
						PDB[atomi].radius = radius[1]; // C3H1
					}
	...
	//	printf("Atom Radius PDB for %i is %f\n",atomi,PDB[atomi].radius);
	}
	//printf("Radii Power Done !\n");
	return;
}

/******************************
 * subroutine index_protein_vcon
 * created 16/02/2001
 ******************************
...
int index_protein_vcon(int PDBtot, struct big_global *big_g) {
	...
	float globalmax[3];
	for (xi = 0; xi < 3; ++xi) {
		globalmin[xi] = big_g->globalmin[xi];
		globalmax[xi] = big_g->globalmax[xi];
	}
	...
	box = (struct atomindex*) calloc(dim * dim * dim, sizeof(struct atomindex));
	PDBlist = (int *) malloc(PDBtot * sizeof(int));
	if ((!box) || (!PDBlist)) {
	...
	for (xi = 0; xi < 3; ++xi) {
		big_g->globalmin[xi] = globalmin[xi];
		big_g->globalmax[xi] = globalmax[xi];
	}
...
int get_contlist4(struct atom PDB[], int atomzero,
	...
	while (currindex != -1) {
		//printf("Currindex:%d\n",currindex);
		PDB[big_g->ca_rec[currindex].atom].done = 'C'; // makes contact
		// if (currindex == big_g->ca_rec[currindex].prev) {break;}
		// printf("Currindex:%d %d\n",currindex, big_g->ca_rec[currindex].prev);
		currindex = big_g->ca_rec[currindex].prev;
	}
	...
	for (i = 0; i < 27; ++i) {
		...
		boxi = boxzero + dim * dim * ((i / 9) - 1) + dim * (((i / 3) % 3) - 1)
				+ (i % 3) - 1;
		if ((boxi < 0) || (boxi >= dim * dim * dim))
			continue; // don't do boxes outside of range
		bai = 0;
		while (bai < box[boxi].nument) {
			...
			sqrdist = (PDB[atomzero].coor[0] - PDB[atomj].coor[0])
					* (PDB[atomzero].coor[0] - PDB[atomj].coor[0])
					+ (PDB[atomzero].coor[1] - PDB[atomj].coor[1])
							* (PDB[atomzero].coor[1] - PDB[atomj].coor[1])
					+ (PDB[atomzero].coor[2] - PDB[atomj].coor[2])
							* (PDB[atomzero].coor[2] - PDB[atomj].coor[2]);
	...
	currindex = big_g->ca_index[atomzero];
	while (currindex != -1) {
		PDB[big_g->ca_rec[currindex].atom].done = 'Y'; // re-mark as done
		currindex = big_g->ca_rec[currindex].prev;
	}
...
void parse_commandline(int argc, char *argv[], char FILENAME[],
	...
	big_g->planedef = 'X'; // default
	big_g->normalize = 'N'; // default
	strcpy(big_g->radfilename, "radii.dat"); // ADDED RJN 28.04.2008
	argi = 1;
	...
	while (argi < argc) {
		if (!strncmp(argv[argi], "-all", 4)) {
			...
			big_g->normalize = 'Y';
		} else if (!strncmp(argv[argi], "-radii", 6)) { // ADDED RJN 28.04.2008
			strcpy(big_g->radfilename, argv[++argi]); // ADDED RJN 28.04.2008
		} else if (!strncmp(argv[argi], "-planedef", 9)) {
			if ((argv[argi][9] == 'X') || (argv[argi][9] == 'R')
					|| (argv[argi][9] == 'B')) {
				big_g->planedef = argv[argi][9];
			} else if ((argi + 1) < argc) {
				if ((argv[argi + 1][0] == 'X') || (argv[argi + 1][0] == 'R')
						|| (argv[argi + 1][0] == 'B')) {
					big_g->planedef = argv[argi + 1][0];
					++argi;
				} else {
					printf(
							"error: need to specify X, R, or B after -planedef switch\n");
					exit(1);
				}
			} else {
				printf(
